...
class Program
{
    ...
    static void ShowCategoryStats(List<Goal> goals, string category)
    {
        var categoryGoals = goals.Where(goal => goal.GetCategory().Equals(category, StringComparison.OrdinalIgnoreCase)).ToList();
        if (categoryGoals.Count > 0)
        {
            Console.WriteLine($"Goals in category '{category}':");
            ListGoals(categoryGoals);
        }
        else
        {
            Console.WriteLine($"No goals found in category '{category}'.");
        }
    }
            ...
            switch (input)
            {
                ...
                case 4:
                    ...
                    for (int i = 1; i < lines.Length; i++)
                    {
                        ...
                        {
                            case "simple":
                                Goals.Add(new Simple(values[1], values[2], double.Parse(values[3]), int.Parse(values[4]), values[5]));
                                break;
                            case "eternal":..
                                Goals.Add(new Eternal(values[1], values[2], double.Parse(values[3]), int.Parse(values[4]), values[5]));
                                break;
                            case "checklist":
                                Goals.Add(new Checklist(values[1], values[2], double.Parse(values[3]), int.Parse(values[4]), int.Parse(values[5]), int.Parse(values[6]), values[7]));
                                break;
                        }
                    }
                    break;
                case 5:
                    ...
                    break;
                case 6:
                    Console.Write("Enter the category to filter by: ");
                    string selectedCategory = Console.ReadLine();
                    ShowCategoryStats(Goals, selectedCategory);
                    break;
                case 7:
                    return;
            }
